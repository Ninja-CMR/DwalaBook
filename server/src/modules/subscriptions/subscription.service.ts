...
import { sendExpirationNotification, sendPaymentConfirmation } from '../notifications/notifications.service';
...
export const activateSubscription = async (
    ...
    await query(
        ...
        [plan, limit, expireAt.toISOString(), userId]
    );
 ...
 */
export const checkExpiringSubscriptions = async (daysBeforeExpiration: number = 7) => {
    ...
    const result = await query('SELECT * FROM USERS WHERE plan_expire_at <= $1', [nextDay.toISOString()]);
    const users = result.rows.filter((u: any) => {
        if (!u.plan_expire_at || u.plan === 'free') return false;
    ...
    for (const user of users) {
        ...
        const sent = await sendExpirationNotification(user, daysBeforeExpiration, user.plan);

        if (sent) {
            // Update last notification timestamp
            await query(
                'UPDATE USERS SET last_notification_sent = $1 WHERE id = $2',
...
export const checkExpiredSubscriptions = async () => {
    ...
    const result = await query('SELECT * FROM USERS WHERE plan_expire_at <= $1', [now]);

    const expiredUsers = result.rows.filter((u: any) => u.plan !== 'free');
